...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const filteredPosts = allPostsData.filter(post => post.Category === currentCategory);
  ...
  if (posts.length === 0) {
    return (
      <div className="pt-20 min-h-screen flex flex-col items-center justify-center text-center px-4">
        <h1 className="text-2xl font-semibold mb-4">{currentCategory} </h1>
  ...
  return (
    <div className="pt-20 overflow-x-hidden">
      <h1 className="text-2xl md:text-3xl font-bold text-left pl-4 sm:pl-8 md:pl-12 mt-12 sm:mt-0 mb-6 py-4 bg-gradient-to-r from-slate-200 via-slate-100/60 to-slate-50/30 border-b-2 border-blue-800 rounded-t-lg text-gray-800 dark:text-gray-100">
        {currentCategory} 
      </h1>
      {posts.map((post, index) => {
        ...
        return (
          <section
            ...
            } items-center justify-center gap-8 p-4 md:p-8`}
          >
...
export async function generateStaticParams() {
  ...
  return uniqueCategories.map(category => ({
    Category: encodeURIComponent(category) // URL用にエンコード
  }));
}