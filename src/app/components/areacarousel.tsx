...
interface AreaCarouselProps {
  ...
  currentContentArea: string; // 現在のメインコンテンツのカテゴリ名
...
const AreaCarousel: React.FC<AreaCarouselProps> = ({
  ...
  const carouselDisplayItems = useMemo(() => {
    if (!allContentItems || !currentContentArea) {
      return [];
    }
    // 1. カテゴリでフィルタリング (大文字・小文字を区別しない)
    const areaFiltered = allContentItems.filter(
      (item) => item.Area.toLowerCase() === currentContentArea.toLowerCase()
    );
    // 2. 現在表示中のアイテムを除外
    const finalFiltered = areaFiltered.filter(
      (item) => item.slug !== currentContentSlug
    );
    // 3. シャッフル
    return shuffleArray(finalFiltered);
  }, [allContentItems, currentContentArea, currentContentSlug]);
  ...
  if (!carouselDisplayItems || carouselDisplayItems.length === 0) {
    ...
    return (
      <div className="w-full py-8 md:py-12 text-center text-gray-500">
        同じ「{currentContentArea}」エリアの他のコンテンツは現在ありません。
      </div>
    );
  }
  ...
  if (!currentItem) { 
      return (
        <div className="w-full py-8 md:py-12 text-center text-gray-500">
            同じ「{currentContentArea}」カテゴリのコンテンツを読み込み中です...
        </div>
      );
  }
            ...
            {carouselDisplayItems.map((item, index) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 p-4 md:p-6 text-white w-full">
                  ...
                  <p className="text-sm md:text-base text-gray-200 mb-2 drop-shadow-md">カテゴリ: {item.Category}</p>
                  <Link
                    href={`/${item.slug}`} // 各コンテンツページへのリンク
